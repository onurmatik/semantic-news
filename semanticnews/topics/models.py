...
class Topic(models.Model):
    ...
    def publish(self):
        ...
        for text in self.texts.filter(status="finished"):
            snapshot = TopicPublicationText.objects.create(
                publication=publication,
                content=text.content,
                created_at=text.created_at,
                updated_at=text.updated_at,
            )
            text_id_map[str(text.id)] = str(snapshot.id)
        ...
        for data in self.datas.all():
            ...
            data_id_map[str(data.id)] = str(snapshot.id)
        ...
        for visualization in self.data_visualizations.all():
            snapshot = TopicPublicationDataVisualization.objects.create(
                publication=publication,
            ...
            visualization_id_map[str(visualization.id)] = str(snapshot.id)

        for layout in self.module_layouts.filter(version=TopicModuleLayout.DRAFT_VERSION):
            module_key = layout.module_key
            base_key, identifier = module_key.split(":", 1) if ":" in module_key else (module_key, None)

            if identifier:
                if base_key == "text":
                    mapped_identifier = text_id_map.get(identifier)
                    if mapped_identifier:
                        module_key = f"{base_key}:{mapped_identifier}"
                elif base_key == "data":
                    mapped_identifier = data_id_map.get(identifier)
                    if mapped_identifier:
                        module_key = f"{base_key}:{mapped_identifier}"
                elif base_key == "data_visualizations":
                    mapped_identifier = visualization_id_map.get(identifier)
                    if mapped_identifier:
                        module_key = f"{base_key}:{mapped_identifier}"
